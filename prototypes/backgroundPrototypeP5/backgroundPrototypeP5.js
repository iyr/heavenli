var cx;
var cy;
var wx  = 140.0;   //width modifier
var wy  = 200.0;   //height modifier
var ang  = 0.0;     //angular offset
var dBias;
var   nb  = 3;       //number of zones
...
function draw() {
  ...
  if (mode > 0) 
    drawHomeCircle(cx, cy, wx, wy, nb, ang);

  else if (mode < 0) {
    drawHomeTri(cx, cy, wx, wy, nb, ang);
    ...
    for (var i = 0; i < nb; i++) {
      stroke(0, 0, 255);
      fill(i*(255/nb));
      var quack = radians(i*(180/constrain(nb-1, 1, nb))+(ang-ang%45));
      ellipse(
  ...
  case 3:
    {
      ...
      if (ao > 337.5 && ao <= 360.0 || ao >= 0 && ao < 22.5) {
        var tmX = [gx, gx+dx, gx+dx, gx+dx-dBias*0.333, gx-dx+dBias*0.333, gx-dx, gx-dx];
      ...
      if (ao > 67.5  && ao <= 112.5) {          
        ...
        var tmY = [gy, gy+dy, gy+dy, gy+dy-dBias*0.333, gy-dy+dBias*0.333, gy-dy, gy-dy];
        ptsX = tmX;
        ptsY = tmY;
      }
